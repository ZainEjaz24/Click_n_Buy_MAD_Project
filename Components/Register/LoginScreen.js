  ...
  return (
    <BackView>
      <KeyboardAvoidingView style={signinStyle.signinView}>
        ...
        {error == "" ? (
          <Text style={{ color: "white", fontSize: 20, fontFamily: "Mogra" }}>
            {loading ? <ActivityIndicator color="#000" size={34} /> : "Signin"}
          </Text>
        ) : (
          <Text
            style={{ color: "red", fontSize: 16, fontFamily: "PoppinsMedium" }}
          >
            {error}
          </Text>
        )}
        ...
        <View style={signinStyle.dontAccount}>
          <Text style={[signinStyle.text, { color: "#fff" }]}>
...
//       if (user) {
//         // Save user token or any other data to AsyncStorage if needed
//         navigation.navigate("first");
//         setLoading(false);
//         setEmail("");
//         setPass("");
